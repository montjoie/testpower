...
do
	...
	cat uuid
	y=$(cut -d _ -f1 uuid)
	file1=$(curl -F "path=@/lava-$y/0/tests/0_server/acme-utils/pyacmecapture/boot_measurements-report.txt" $ARTI || exit $?)
	...
	file3=$(curl -F "path=@/lava-$y/0/tests/0_server/acme-utils/pyacmecapture/boot_measurements_Slot_$probe_channel.csv" $ARTI || exit $?)
	lava-test-reference file3 --result pass --reference $file3
	file4=$(curl -F "path=@/lava-$y/0/tests/0_server/acme-utils/pyacmecapture/test_measurements_Slot_$probe_channel.csv" $ARTI || exit $?) 
